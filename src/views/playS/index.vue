<template>
  <div>
    <div class="box" v-for="(item, index) in musicurl.data" :key="index">
      <audio :src="item.url" ref="audio"></audio>
    </div>
    <div class="plays">
      <div
        class="playsbg"
        ...
        :key="index"
        :style="{ background: 'url(' + item.al.picUrl + ')' }"
      ></div>
      <div class="m-logo">
        <svg
...
c4.271-0.012,7.729-3.47,7.74-7.74V7.74C33.989,3.47,30.53,0.011,26.26,0"
      ...
      <div class="Cover">
        ...
        >
          ...
          <span class="playbtn" @click="playbtn"></span>
        </div>
        <div
          class="Author"
          v-for="(item, index) in musiclist.songs"
    ...
    getmusicurl() {
      ...
      this.$http.get("/song/url?id=" + id + "").then((res) => {
        ...
        if (res.status == 200) {
          ...
          console.log(this.musicurl, "歌曲musicurltv名称");
        } else {
          this.$Message.error(res.msg);
        }
      });
...
.plays {
  ...
  .playsbg {
    ...
    overflow: hidden;
    z-index: 1;
    // background-image: url(http://p1.music.126.net/cB1-Z7oa-onqUr_xEIY6Eg==/109951165592399709.jpg?imageView&thumbnail=50y50&quality=15&tostatic=0);
    opacity: 1;
    transform: scale(1.5);
